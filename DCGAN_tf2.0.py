...
# 模型参数
class get_defalut_params(object):
    z_dim=100,                          # 随即向量长度，<向量，图像>，要重排成矩阵才可以反卷积
    init_conv_size=4,                   # 向量变成矩阵的大小？初始化卷积核大小
    g_channels=[128, 64, 32, 1],        # 生成器G，各个反卷积层的通道数目 mnist是单通道，所以最后是1                                         # 其反卷积层大小，步长等也应该定义，为了简单后面统一
...
mnist_dadta = MnistData(mnist.train.images, hps.z_dim[0], hps.img_size[0])
...
class DCGAN(object):
    def __init__(self, hps):
        g_channels = hps.g_channels[0]
        d_channels = hps.d_channels[0]
        self._batch_size = hps.batch_size[0]
        self._init_conv_size = hps.init_conv_size[0]
        self._z_dim = hps.z_dim[0]
        self._img_size = hps.img_size[0]
...
train_op = dcgan.build_train_op(losses, hps.learning_rate[0], hps.beta1[0])
...
with tf.Session() as sess:
    ...
    for step in range(train_steps):
        batch_imgs, batch_z = mnist_dadta.next_batch(hps.batch_size[0])
        ...
        if should_sample:                                       # 输出图像
            ...
            # 小图像拼成大图像
            gen_img = combine_imgs(gen_imgs_val, hps.img_size[0])
            ground_img = combine_imgs(batch_imgs, hps.img_size[0])
            # 保存，输出图像
